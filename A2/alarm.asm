...
MVI A,00H
MVI B,00H
; taking input for alarm time in hh:mm format
CALL GTHEX
; storing alarm time in hl register pair
MOV H,D
MOV L,E
SHLD 8000H
...
CALL GTHEX
; storing input time in hl register pair						
MOV H,D 						
MOV L,E
...
;to check if curr time matches the alarm time
CHECK:
    MOV C,A
    ...
    ;checking if time matches curr time
    MOV A,D
    CMP H
    JNZ EXIT
